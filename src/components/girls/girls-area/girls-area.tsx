          ...
          <div className="row">
            <div className="col-lg-12">
              <div className="tp-section text-center">
                ...
                <div className="tp-section text-center">
                  {(selected_category === 'ALL' || selected_category === '2012 - 2013') && (
                    <div className="row">
                      <h3 className={cn('tp-section__title mb-70', s.title)}>{t('year2012_2013')}</h3>
                      {girls_data_2012_2013.map((item) => {
                        const name = t(`names.${item.id}`, {defaultValue: item.name})
                        const title = t(`titles.${item.id}`, {defaultValue: item.title})
                            ...
                            <div className="team-item mb-35">
                              ...
                              <div className="team-item__content">
                                <h5 className="team-item__title mb-15">
                                  <Link href={`/girls/${item.id}`}>{name}</Link>
                                </h5>
                                <span>{title}</span>
                              </div>
                            </div>
                  ...
                  {(selected_category === 'ALL' || selected_category === '2010 - 2011') && (
                    <div className="row">
                      <h3 className={cn('tp-section__title mb-70', s.title)}>{t('year2010_2011')}</h3>
